  ...
  return (
    <section className="mb-16 animate-fade-in-up">
      <h2 className="text-3xl font-bold mb-6 flex items-center text-white">
        <span className="text-accent mr-4">*</span> {title}
      </h2>
      <div className="space-y-8">
        {items.map((item, index) => (
          <div key={item.title} className="group">
            <Link href={item.href} target="_blank">
              <h3 className="text-xl font-semibold mb-1 text-white group-hover:text-[var(--accent-color)] transition-colors duration-200">
                {item.title}
              </h3>
